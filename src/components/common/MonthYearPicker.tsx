...
export const MonthYearPicker: React.FC<MonthYearPickerProps> = ({
  ...
  const handleSelect = (month: number | null, year: number) => {
    ...
    if (year > currentYearNow) return;
    if (year === currentYearNow && month !== null && month > currentMonthNow) return;
    ...
    <div className="w-full group" ref={containerRef}>
      ...
      <div className="relative">
        <button
          ...
          } ${value ? 'text-slate-900 dark:text-white' : 'text-slate-400'} ${className}`}
        >
          ...
          {isOpen && (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6 px-2">
                ...
                <button 
                  type="button"
                  onClick={() => setViewYear(Math.min(viewYear + 1, currentYearNow))}
                  disabled={viewYear >= currentYearNow}
                  className={`p-2 rounded-xl transition-colors ${
                    viewYear >= currentYearNow 
                      ? 'text-slate-200 dark:text-slate-800 cursor-not-allowed' 
                      : 'hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-400 hover:text-blue-600'
                  }`}
                >
              ...
              <div className="grid grid-cols-3 gap-2">
                {MONTHS.map((month, idx) => {
                  ...
                  const isFuture = viewYear === currentYearNow && idx > currentMonthNow;
