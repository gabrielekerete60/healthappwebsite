...
export default function IdentityVerificationStep({ formData, handleUpdate, validationErrors }: IdentityVerificationStepProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsUploading(true);
    try {
      // Phase 14: Client-side storage protection
      const MAX_SIZE = 10 * 1024 * 1024; // 10MB
      const MIN_SIZE = 1024; // 1KB (prevents corrupted/empty files)

      if (file.size > MAX_SIZE) {
        throw new Error("File is too large. Maximum size is 10MB.");
      }
      if (file.size < MIN_SIZE) {
        throw new Error("File is too small. Please upload a clear document image or PDF.");
      }
