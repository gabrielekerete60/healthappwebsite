...
export default function AuthActions({ user, loading, locale, t, setTheme, resolvedTheme, mounted, isAdminAuthenticated: initialAdminAuth }: AuthActionsProps) {
  const router = useRouter();
  const pathname = usePathname();
  const [userProfile, setUserProfile] = React.useState<UserProfile | null>(null);
  const [isAdminAuthenticated, setIsAdminAuthenticated] = React.useState(initialAdminAuth || false);

  React.useEffect(() => {
    // Check for admin session independently of Firebase
    const adminSession = sessionStorage.getItem('isAdminAuthenticated') === 'true';
        ...
        <div className="flex items-center gap-2 xl:gap-3">
          {(userProfile?.role === 'admin' || isAdminAuthenticated) ? (
            <Link 
              ...
              className="flex items-center gap-2.5 px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl hover:scale-[1.02] active:scale-95 transition-all shadow-xl shadow-slate-900/10 dark:shadow-none group"
            >
              <div className="w-5 h-5 bg-blue-600 rounded-lg flex items-center justify-center shadow-lg shadow-blue-500/20 group-hover:rotate-12 transition-transform duration-500">
                <ShieldCheck className="w-3 h-3 text-white" />
              </div>
              <span className="text-[10px] font-black uppercase tracking-[0.2em]">{t('header.adminConsole')}</span>
            </Link>
          ) : (
            <>
