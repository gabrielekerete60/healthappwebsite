...
export function VerificationCard({ id, label, value, icon, status, otpValue = "", setOtpValue, onSend, onVerify, onReset, isResumed, actionLabel, t }: VerificationCardProps) {
  ...
  const tDefault = (key: string) => t ? t(key) : key;

  // Auto-verify when OTP reaches 6 digits
  React.useEffect(() => {
    if (otpValue.length === 6 && isSent) {
      onVerify();
    }
  }, [otpValue, isSent, onVerify]);
          ...
          <AnimatePresence mode="wait">
            {isVerified ? (
              ...
              >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">{tDefault('verifiedMember')}</span>
              </motion.div>
            ) : (
              <motion.div 
                key="actions"
                ...
                {isSent && (
                  <div className="flex-1 flex items-center gap-3">
                    <input
                      ...
                      placeholder="------"
                      autoFocus
                      className="w-full sm:w-32 px-4 py-3.5 bg-slate-50 dark:bg-slate-800 border-2 border-blue-500 rounded-xl text-center font-mono font-black text-lg tracking-[0.3em] outline-none shadow-inner focus:ring-4 focus:ring-blue-500/10 transition-all"
                    />
                    <motion.button 
                  ...
                  <motion.button 
                    ...
                    className={`flex-1 xl:flex-none px-6 sm:px-8 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-3 xl:min-w-[180px] ${
                      isSending
                        ...
                        : 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-lg hover:shadow-blue-500/20 border-2 border-slate-100 dark:border-white/5'
                    }`}
                  >
                    {isSending ? (
